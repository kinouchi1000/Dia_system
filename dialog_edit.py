...
def set_logger(name, directoryName="Data/main/"):
    dt_now = datetime.now()
    dt = dt_now.strftime('%Y%m%d_%H%M%S')
    fname = directoryName + dt
...
class FavotModel(object):
    ...
    def __init__(self, args, *, logger=None):
        ...
        print(args.batch_size, args.buffer_size,
        ...
        if self.cfg.interactive.buffer_size > 1:
            logger.info("Sentence buffer size: %s",
                        self.cfg.interactive.buffer_size)
...
class Favot(object):
    ...
    def __init__(self, args, favot_model, *, logger=None, parser=None):
        ...
        self.debug = False
        self.delimiter = "．。 　?？!！♪☆★"
        self.sent_splitter = re.compile(
            ".*?[{}]".format(self.delimiter), re.DOTALL)
    ...
    def set_generator_parameters(self, args):
        for k, v in args.items():
            ...
            print(self.args)
        self.fm.generator = self.fm.task.build_generator(
            self.fm.models, self.args)
    ...
    def _execute(self, uttr, **kwargs):
        ...
        for i, batch in enumerate(make_batches(inputs, args, task, max_positions, self.encode_fn)):
            ...
            translations = task.inference_step(
                self.fm.generator, self.fm.models, sample, constraints=constraints)
            ...
            for i, (id, hypos) in enumerate(zip(batch.ids.tolist(), translations)):
                src_tokens_i = utils.strip_pad(
                    src_tokens[i], self.fm.tgt_dict.pad())
        ...
        for id_, src_tokens, hypos, info in sorted(results, key=lambda x: x[0]):
            if self.fm.src_dict is not None:
                src_str = self.fm.src_dict.string(
                    src_tokens, args.common_eval.post_process)
                #src_str = self.fm.src_dict.string(src_tokens, args.post_process)
                print("W-{}\t{:.3f}\tseconds".format(id_, info["time"]))
                for constraint in info["constraints"]:
                    print("C-{}\t{}".format(id_, self.fm.tgt_dict.string(constraint,
                          args.common_eval.post_process)))
                    if self.debug:
            ...
            for i, hypo in enumerate(hypos[:min(len(hypos), min(args.generation.nbest, self.args.show_nbest))]):
                ...
                dup_flag, nodup_cand = self.contain_duplicate(
                    detok_hypo_str, mode=mode, id=id_)
                #ret_scores[detok_hypo_str] = score - 10000
                if dup_flag and self.args.suppress_duplicate:
                    self.logger.info(
                        "duplicated pattern: {}".format(detok_hypo_str))
                    if nodup_cand != "":
                ...
                self.logger.info("system_utt_cands: " +
                                 'H-{}\t{}\t{}'.format(id_, score, hypo_str))
                self.logger.info(
                    "system_utt_cands: " + 'D-{}\t{}\t{}'.format(id_, score, detok_hypo_str))
                if self.debug:
                ...
                if "unk" in detok_hypo_str and len(_contexts) > 0:
                    ...
                    self.logger.info(
                        "{}/{}/{}".format(str(c1), str(c2), str(c3)))
                    try:
                        if len(c1) > 0:
                            c1 = c1[0]
                            cc = re.findall("{}(.)".format(
                                c1[0:2]), _contexts[-1]["utt"], re.DOTALL)
                            if len(cc) > 0:
                                detok_hypo_str = detok_hypo_str.replace(
                                    c1[0:2] + "<unk>", c1[0:2] + cc[0])
                        elif len(c2) > 0:
                            c2 = c2[0]
                            cc = re.findall("{}(.){}".format(
                                c2[0], c2[1]), _contexts[-1]["utt"], re.DOTALL)
                            if len(cc) > 0:
                                detok_hypo_str = detok_hypo_str.replace(
                                    c2[0] + "<unk>" + c2[1], c2[0] + cc[0] + c2[1])
                        elif len(c3) > 0:
                            c3 = c3[0]
                            cc = re.findall("(.){}".format(
                                c3[0:2]), _contexts[-1]["utt"], re.DOTALL)
                            if len(cc) > 0:
                                detok_hypo_str = detok_hypo_str.replace(
                                    "<unk>" + c3[0:2], cc[0] + c3[0:2])
                        else:
                            score -= 1000
                    except:
                        score -= 1000
                ...
                nodup_cand = nodup_cand.replace("(笑)", " ").replace(
                    "(笑）", " ").replace("（笑)", " ")
    ...
    def contain_duplicate(self, hypo, mode="normal", id=-1):
        ...
        for orgs in sents:
            ...
            for i, cdic in enumerate(sent_contexts[::-1]):
                ...
                if e > 0.5:
                    self.logger.info(
                        "sim: {}, cand: {}, contexts: {}".format(e, s, c))
                if e > 0.65:
                    f = True
                    ff = True
                    break
        ...
        for cdic in contexts:
            ...
            if e > 0.5:
                self.logger.info(
                    "all sim: {}, cand: {}, contexts: {}".format(e, hypo, c))
            if e > 0.5:
                f = True
                ff = True
                break
        ...
        for i, s in enumerate(nodup_cand):
            ...
            for j, cdic in enumerate(_contexts):
                ...
                if e > 0.5:
                    self.logger.info(
                        "self: sim: {}, cand: {}, contexts: {}".format(e, s, c))
                if e > 0.65:
                    f = True
                    ff = True
                    break
...
def add_local_args(parser):
    ...
    parser.add_argument('--use-convert-kakikotoba',
                        default=True, type=bool, help='Use convertor 書き言葉')
...
def main():
    # log出力
    logger = set_logger("dialog", "log/dialog/")
    # オプション設定
