...
class Centering_Theory:
    ...
    def Centering_Word(self,sentence,usr_id):
        ...
        if self.ZeroPart==0:
            ...
            self.Objectum(sentence)            #目的語の先行詞の探索
        self.ZeroPart=0
        return sentence
    ...
    def Follow_Zero_Part(self,sentence,usr_id):
        ...
        if Main_part==False:
            ...
            if Do_part==True or Nane_follow_part==True:
                ...
                if Cb!='': 
                    sentence = self.Follow_word(sentence,Cb,fell_part,'は',None,usr_id)
                    self.ZeroPart=0
                else:
                    if particle_part==True:
                        self.ZeroPart=0
                    else:
                        self.ZeroPart=1
            ...
            if Ob_Follow==False:
                ...
                if self.O_index!=0:
                    ...
                    if Do_word!='' and Ob_word not in sentence and len(Ob_word)!=0:
                        self.logger.add_log('Pair:(' + str(Ob_word) + ' → ' + str(Do_word) +')') 
                        frame=self.SQ_usr.check(Do_word,Ob_word)
                        self.logger.add_log('frame->'+str(frame))

                        # 格助詞が判明した場合、目的語を補う
                        if frame!=0:
                            sentence = self.Follow_word(sentence,Ob_word,fell_part,frame,Do_word_all,usr_id)   
        ...
        for i,word in enumerate(wakati):
            ...
            if word_class=='助詞' and word in particle_words:
                if '終助詞' not in part[i][1] and '接続助詞' not in part[i][1]:
                    # 補完する対象が助詞の場合は除く
                    if pre_word not in self.AVOID_WORD_LIST and pre_part!='助詞' and pre_part!='助動詞':
                        # 「の、こと、もの」
                        ...
                        # サ変接続+「する」を結合
                        if self.mecab_user.get_original_word(wakati[i - 2])=='する' and thing_flag==True:
                            if part[i - 3][0]=='名詞' and part[i - 3][1]=='サ変接続':
                                # print('サ変接続+「する」')
                                word_thing = wakati[i - 3] + word_thing

                        # 「形容動詞語幹+助動詞」
                        elif wakati[i - 2]=='な' and part[i-2][0]=='助動詞' and thing_flag==True:
                            if part[i - 3][0]=='名詞' and part[i - 3][1]=='形容動詞語幹':
                                # print('形容動詞語幹+助動詞')
                                word_thing = wakati[i - 3] + word_thing
                        #動詞「った」
                        elif wakati[i - 2]=='た' and part[i-2][0]=='助動詞' and thing_flag==True:
                             if part[i - 3][0]=='動詞' and part[i - 3][1]=='自立':
                                # print('動詞+自立')
                                word_thing = wakati[i - 3] + word_thing
        ...
        for i in range(len_word):
            ...
            if len(self.Cp_list)!=0:
                if self.Cp_list[self.index-1] not in Fo1_word:
                    ...
                    d_wakati,d_part = self.mecab_user.get_word_part_list(Fo2_word)
                    ...
                    if '動詞' in d_part or '形容詞' in d_part or '助動詞' in d_part:
                        ...
                        for i in range(len_w_part):
                            ...
                            if part=='助詞' and p_part=='名詞':
                                # 先行詞には用いない単語を探索
                                if w_word[i-1] not in self.AVOID_WORD_LIST and w_word[i-1] not in self.Not_noun_words:
                                    Ob_word=''
                                    # 「の、こと、もの」
...
if __name__ == "__main__":
    ...
    with open("onlyWrite.txt","r") as f:
        with open("centered.txt","w") as wf:
            for line in f.readlines():
                wf.write(Center.Centering_Word(line.rstrip('\n'),usr)+"\n")
